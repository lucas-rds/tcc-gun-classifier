...
def loadData(img_size, classSize):
    guns = glob.glob('./test-gun/binary_gun_dataset/**/*.jpg', recursive=True)
    notGuns = glob.glob('./test-gun/binary_notgun_dataset/**/*.jpg', recursive=True)
    # guns = glob.glob('./gun-dataset/**/*.jpg', recursive=True)
    # notGuns = glob.glob('./not-gun-dataset/**/*.jpg', recursive=True)
    
    imgSize = (img_size, img_size)
    xGun, yGun = loadImgClass(guns, 0, classSize, imgSize)
    xNotGun, yNotGun = loadImgClass(notGuns, 1, classSize, imgSize)
    print("There are", len(xGun), "gun images")
    print("There are", len(xNotGun), "not gun images")
    
    X = np.array(xGun + xNotGun)
    y = np.array(yGun + yNotGun)
...
def karasModel(inputShape):
    ...
    model.add(Convolution2D(16, 8, 8, subsample=(4, 4),
                            border_mode='valid', input_shape=inputShape))
